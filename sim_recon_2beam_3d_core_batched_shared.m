function reconBatch = sim_recon_2beam_3d_core_batched_shared(raw_batch, otf_data, params)
% SIM_RECON_2BEAM_3D_CORE_BATCHED_SHARED
% Batched single-GPU/CPU reconstruction for a group of same-size tiles that
% share the same accepted reconstruction parameters.
%
% raw_batch:
%   [ny, nx, nz*nphases*ndirs, nbatch]
%
% otf_data:
%   pre-resampled or canonical OTF matching the tile size.
%
% Important:
%   This batched path assumes all tiles in the batch share the same params
%   that affect separation, shifting, modulation handling, and assembly.
%   In practice the unified tiled wrapper only uses this path for tiles
%   whose accepted parameters have the same signature.
%
% Fidelity:
%   - same separation model
%   - same overlap-equalized modulation estimator
%   - same Wiener assembly
%   - same apodization model
%
% Limitation:
%   This batched path requires params.k0_search = false so the same carrier
%   shift is used for the whole batch. That is the recommended tile default.
...
if ~isfield(params, 'k0_search'),           params.k0_search = false;     end
...
assert(~params.k0_search, 'Batched shared-parameter path requires params.k0_search = false.');
...
if useGPU
    g = gpuDevice();
    fprintf('Single-GPU batched reconstruction: %s (%.1f GB free)\n', g.Name, g.AvailableMemory/1e9);
    toDevice = @(x) gpuArray(single(x));
else
    toDevice = @(x) single(x);
end

assert(ndims(raw_batch) == 4, 'raw_batch must be [ny nx nimgs nbatch].');
[ny_raw, nx_raw, nimgs, nbatch] = size(raw_batch);
nz = nimgs / (nphases * ndirs);
assert(mod(nz,1) == 0, 'raw_batch size incompatible with nphases*ndirs');
nz = round(nz);
ny_out = ny_raw * zoomfact;
nx_out = nx_raw * zoomfact;

raw_batch = single(raw_batch) - single(params.background);

if params.fastSI
    data_6d = reshape(raw_batch, ny_raw, nx_raw, nphases, ndirs, nz, nbatch);
    data_6d = permute(data_6d, [1 2 5 3 4 6]); % [ny nx nz phase dir batch]
else
    data_6d = reshape(raw_batch, ny_raw, nx_raw, nphases, nz, ndirs, nbatch);
    data_6d = permute(data_6d, [1 2 4 3 5 6]); % [ny nx nz phase dir batch]
end
clear raw_batch;
...
numerator   = complex_zeros_like_device([ny_out, nx_out, nz, nbatch], useGPU);
denominator = real_zeros_like_device([ny_out, nx_out, nz, nbatch], useGPU);
...
for idir = 1:ndirs
    data_dir = toDevice(data_6d(:,:,:, :, idir, :)); % [ny nx nz phase 1 batch]
    data_dir = reshape(data_dir, ny_raw, nx_raw, nz, nphases, nbatch);

    if params.usePerDecomp
        phase_ft = complex_zeros_like_device([ny_raw, nx_raw, nz, nphases, nbatch], useGPU);
        for iph = 1:nphases
            vols = reshape(data_dir(:,:,:,iph,:), ny_raw, nx_raw, nz, nbatch);
            for ib = 1:nbatch
                vol = perdecomp_3d_apply(vols(:,:,:,ib), perdenom);
                phase_ft(:,:,:,iph,ib) = fftshift3(fft3_spatial(vol));
            end
        end
    else
    ...
    bands = batch_separate_batched(phase_ft, sep_inv_dev_all{idir});
    clear phase_ft;
    ...
    band0_up   = upsample_band_3d_batch_gpu(reshape(bands(:,:,:,2,:), ny_raw, nx_raw, nz, nbatch), ny_out, nx_out);
    band_p1_up = upsample_band_3d_batch_gpu(reshape(bands(:,:,:,3,:), ny_raw, nx_raw, nz, nbatch), ny_out, nx_out);
    band_m1_up = upsample_band_3d_batch_gpu(reshape(bands(:,:,:,1,:), ny_raw, nx_raw, nz, nbatch), ny_out, nx_out);
    clear bands;

    shift_pos = build_shift_cache(k0_up, shift_grid);
    shift_neg = build_conjugate_shift_cache(shift_pos);

    band_p1_shifted = apply_shift_cache_nd(band_p1_up, shift_pos);
    band_m1_shifted = apply_shift_cache_nd(band_m1_up, shift_neg);
    otf1_shifted    = apply_shift_cache_nd(otf1_up, shift_pos);
    otf_m1_shifted  = apply_shift_cache_nd(conj(otf1_up), shift_neg);
    clear band_p1_up band_m1_up otf1_up;
    ...
    [mod_amp_vec, ~] = estimate_modamp_overlap_equalized_batch(band0_up, band_p1_shifted, ...
        otf0_up, otf1_shifted, params.otf_support_thresh);
    mod_amp_eff = choose_modamp_batch(mod_amp_vec, idir, params);

    amp_mag2 = reshape(single(max(abs(mod_amp_eff).^2, eps('single'))), 1, 1, 1, nbatch);
    amp_conj = reshape(single(conj(mod_amp_eff)), 1, 1, 1, nbatch);
    amp_fwd  = reshape(single(mod_amp_eff), 1, 1, 1, nbatch);

    weight_m1 = noiseWeightsDev_all{idir}(:,:,:,1) .* center_weight_m1 .* cudaFilter.zmask1_4d;
    weight_0  = noiseWeightsDev_all{idir}(:,:,:,2) .* center_weight_0  .* cudaFilter.zmask0_4d;
    weight_p1 = noiseWeightsDev_all{idir}(:,:,:,3) .* center_weight_p1 .* cudaFilter.zmask1_4d;
    ...
    if params.amp_in_wiener
        ...
        band_p1_shifted = band_p1_shifted ./ reshape(mod_amp_eff, 1, 1, 1, nbatch);
        band_m1_shifted = band_m1_shifted ./ conj(reshape(mod_amp_eff, 1, 1, 1, nbatch));
...
if params.suppress_dc
    ...
    cz = floor(nz/2) + 1;
    for ib = 1:nbatch
        denominator(cy, cx, cz, ib) = max(denominator(:,:,:,ib), [], 'all') * single(0.1);
    end
end
...
reconBatch = real(ifft3_spatial(ifftshift3(recon_ft)));
if useGPU
    reconBatch = gather(reconBatch);
end
reconBatch = single(reconBatch);
end

function bands = batch_separate_batched(phase_ft, sep_inv)
% phase_ft: [ny nx nz nph batch]
[ny, nx, nz, nph, nb] = size(phase_ft);
tmp = permute(phase_ft, [1 2 3 5 4]); % [ny nx nz batch nph]
P = reshape(tmp, [], nph);
B = P * sep_inv.';
tmp2 = reshape(B, ny, nx, nz, nb, nph);
bands = permute(tmp2, [1 2 3 5 4]);   % [ny nx nz nph batch]
end

function out = upsample_band_3d_batch_gpu(band_in, ny_out, nx_out)
% band_in: [ny nx nz nb]
[ny_in, nx_in, nz, nb] = size(band_in);
if ny_in == ny_out && nx_in == nx_out
    out = band_in;
    return;
end
if isa(band_in, 'gpuArray')
    out = complex(gpuArray.zeros(ny_out, nx_out, nz, nb, 'single'));
else
    out = complex(zeros(ny_out, nx_out, nz, nb, 'single'));
end
...
function shifted = apply_shift_cache_nd(vols, cache)
% Supports 3D, 4D, or 5D arrays; shifts only dims 1:3
shiftVec = [cache.int_shift, zeros(1, max(0, ndims(vols)-3))];
shifted = circshift(vols, shiftVec);
if cache.has_frac
...
function [mod_amp_vec, mod_diag] = estimate_modamp_overlap_equalized_batch(band0_up, band1_shifted, otf0_up, otf1_shifted, supportThresh)
% band0_up, band1_shifted: [ny nx nz nb]
a0 = abs(otf0_up);
a1 = abs(otf1_shifted);
mask = (a0 > supportThresh * max(a0(:))) & (a1 > supportThresh * max(a1(:)));
n_mask = nnz(maybe_gather(mask));
nb = size(band0_up, 4);
mod_diag = struct('n_mask', n_mask);

if n_mask < 10
    mod_amp_vec = ones(1, nb, 'single');
    return;
end
...
mask4 = reshape(mask, size(mask,1), size(mask,2), size(mask,3), 1);
xsy = squeeze(maybe_gather(sum(sum(sum(conj(ov0).*ov1.*mask4, 1), 2), 3)));
sx  = squeeze(maybe_gather(sum(sum(sum(abs(ov0).^2.*mask4, 1), 2), 3)));
sy  = squeeze(maybe_gather(sum(sum(sum(abs(ov1).^2.*mask4, 1), 2), 3)));

mod_amp_vec = ones(1, nb, 'single');
for ib = 1:nb
    if sx(ib) <= eps('single') || sy(ib) <= eps('single')
        mod_amp_vec(ib) = single(1);
        continue;
    end
    beta = 0.5 * atan2(2 * abs(xsy(ib)), (sx(ib) - sy(ib)));
    if beta < 0
        beta = beta + 0.5*pi;
    end
    mod_amp = tan(beta) * exp(1i * angle(xsy(ib)));
    mag = abs(mod_amp);
    if mag <= eps('single')
        mod_amp_vec(ib) = single(1);
    else
        mod_amp_vec(ib) = single(mod_amp / mag * min(mag, 1.0));
    end
end
end

function mod_amp_eff = choose_modamp_batch(mod_amp_vec, idir, params)
phase_est = angle(mod_amp_vec);
mag_est = abs(mod_amp_vec);
...
if ~isempty(params.forcemodamp)
    ...
    if ~isreal(fm_val)
        mod_amp_eff = single(repmat(fm_val, size(mod_amp_vec)));
    else
